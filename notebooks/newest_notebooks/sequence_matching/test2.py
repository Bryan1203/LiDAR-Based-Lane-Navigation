...
class FrameSynchronizer:
    def __init__(self):
        ...
        self.cloud_buffer = deque(maxlen=200)  # Adjust based on memory constraints
    ...
    def find_closest_cloud(self, odom_time):
        ...
        # More relaxed buffer range check
        buffer_window = self.max_threshold * 2
        if odom_time < buffer_start - buffer_window or odom_time > buffer_end + buffer_window:
        ...
        if len(self.time_differences) > 10:
            ...
            self.adaptive_threshold = max(self.min_threshold, 
                                        min(self.max_threshold, median_diff * 2.0))
